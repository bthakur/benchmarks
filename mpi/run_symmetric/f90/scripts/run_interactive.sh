...
echo "+----------------------------------+"
  ...
  genv=" "
  envh="-env I_MPI_DEBUG 1"
  envp="-env I_MPI_DEBUG 1 "\\" \n
        -env LD_LIBRARY_PATH $MIC_LD_LIBRARY_PATH "\\" \n
       "\\" \n"

# Creating BASH script for running the job
  ...
  #I_MPI_MIC changes the pmi_proxy executable
  xconfig="$xconfig  \n
  ...
  xconfig="$xconfig \n mpiexec.hydra "\\" \n"
  xconfig="$xconfig $genv "\\" \n"

  for h in $(cat $PbsHosts); do
   envhost=" $envh -n $nperhost -host $h $exe_host   : "\\" \n"
   xconfig="$xconfig $envhost "\\" \n"
  done

  for p in $(cat $PbsPhis); do
   envxphi=" $envp -n $nperxphi -host $p $exe_mic   : "\\" \n"
   xconfig="$xconfig $envxphi "\\" \n"
  done

  xconfig=$(echo ${xconfig%:*})
  echo -e "$xconfig" > hostconfig
  sed "s/ $//g" hostconfig > run
  cat run
  echo "+-Running"
  sh run
  exit
