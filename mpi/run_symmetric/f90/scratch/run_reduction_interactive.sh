...
envhost1="-genv I_MPI_DEBUG 2 -env I_MPI_FABRICS ofa -env I_MPI_OFA_ADAPTER_NAME mlx4_0"
...
envphi1="-env LD_LIBRARY_PATH $MIC_LD_LIBRARY_PATH -env I_MPI_FABRICS shm:ofa -env I_MPI_OFA_ADAPTER_NAME mlx4_0"
...
echo "+-----------------------+"
    ...
    #                   $CODE_TOP/src/reduce_mpi.f90 -o $CODE_TOP/bin/reduce"
...
echo "+---------------------------+"
    ...
    #comm_compile_phi="$MPI_FC -fc=ifort -mmic $CODE_TOP/reduce_mpi.f90 -o $CODE_TOP/reduce.mic"
...
echo "+----------------------"
    ...
    export I_MPI_MIC=0
    export I_MPI_MIC_ENABLE="disable"
    envhost="$envhost1"
    #$MPI_RUN $envhost2 -n $PBS_NP -hostfile $PbsHosts $CODE_TOP/hello
    # note that without perhost, it still launches 2 procs on same node !
    $MPI_RUN $envhost -perhost 1 -n $PBS_NUM_NODES -machinefile $PbsHosts -print-rank-map  $CODE_TOP/bin/reduce
    sleep 1; wait
    #$MPI_RUN  -n $PBS_NP -hostfile $PbsHosts $CODE_TOP/reduce
    exit

echo "+------------------------------------+"
echo "| Running Natively on a Phi: shm:tcp |"
echo "+------------------------------------+"
    export I_MPI_DEBUG=2
    export I_MPI_MIC=1
    export I_MPI_MIC_ENABLE="enable"
    ssh $aphi "$MPI_RUN_PHI $envphi2 -n 2 $CODE_TOP/reduce.mic"
    # The following hangs the nodes
    #ssh $aphi "$MPI_RUN_PHI -n 4 -hostfile $PbsPhis $CODE_TOP/hello.mic"
    sleep 2; wait

echo "+-----------------------------------+"
echo "| Running on Hosts + Phis shm:dapl  |"
echo "+-----------------------------------+"
...
envphi="$envphi2"
    ...
    mpiifort --version
     #mpirun \
     mpiexec.hydra \
          $envhost -n 1 -host $ahost $CODE_TOP/reduce \
         : $envhost -n 1 -host $bhost $CODE_TOP/reduce \
         : $envphi -n 1 -host $aphi  $CODE_TOP/reduce.mic \
         : $envphi -n 1 -host $bphi  $CODE_TOP/reduce.mic 
#exit
    #mpirun $envhost -n 1 -host $ahost $CODE_TOP/hello \
    #     : $envhost -n 1 -host $ahost $CODE_TOP/hello \
    #     : $envphi -n 1 -host $aphi  $CODE_TOP/hello.mic \
    #     : $envphi -n 1 -host $bphi  $CODE_TOP/hello.mic \
    #mpirun  -n 1 -host $ahost $CODE_TOP/hello \
    #     :  -n 1 -host $ahost $CODE_TOP/hello \
    #     :  -n 1 -host $aphi  $CODE_TOP/hello.mic \
    #     :  -n 1 -host $bphi  $CODE_TOP/hello.mic \
